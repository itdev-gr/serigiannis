    ...
    <div className="max-w-3xl">
      ...
      <form action={upsertPattern} className="mt-6 grid gap-5 rounded-lg border border-border bg-surface p-6">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <label className="block text-[13px] text-muted">Γραμμή
            <select name="route_id" defaultValue={pattern?.route_id ?? ''} required className={adminInput}>
          ...
          <label className="block text-[13px] text-muted">Λεωφορείο (διάταξη)
            <select name="layout_id" defaultValue={pattern?.layout_id ?? ''} required className={adminInput}>
              <option value="">— Επιλέξτε —</option>
              {layouts.map((l) => <option key={l.id} value={l.id}>{l.name}</option>)}
            </select>
          </label>
          <label className="block text-[13px] text-muted">Ώρα αναχώρησης
            <input type="time" name="departure_time" defaultValue={pattern?.departure_time?.slice(0, 5) ?? ''} required className={adminInput} />
          </label>
          <div className="grid grid-cols-2 gap-4">
            <label className="block text-[13px] text-muted">Ισχύει από
              <input type="date" name="valid_from" defaultValue={pattern?.valid_from ?? new Date().toISOString().slice(0, 10)} required className={adminInput} />
            </label>
            <label className="block text-[13px] text-muted">Έως (προαιρετικό)
              <input type="date" name="valid_to" defaultValue={pattern?.valid_to ?? ''} className={adminInput} />
            </label>
          </div>
        </div>
        ...
        <label className="block text-[13px] text-muted">Σημειώσεις
          <input name="notes" defaultValue={pattern?.notes ?? ''} className={adminInput} />
        </label>
      ...
      <div className="mt-10 rounded-lg border border-border bg-surface p-6">
        ...
        <form action={createTrip} className="mt-4 grid gap-3 sm:grid-cols-[1fr_1fr_8rem_7rem_auto]">
          <select name="route_id" required className={adminInput}>
